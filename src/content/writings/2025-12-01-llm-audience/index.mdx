...
import photo1 from './llm-pretraining.png';
...
<figure class="my-8">
  ...
  <figcaption class="text-center text-sm text-gray-600 mt-2">LLMs rely on internet writings for their pre-training data</figcaption>
</figure>
...
(illustration LLM reading google review then recommending restaurant)
...
(illustration LLM walking through small andean town)
...
(illustration LLM with good and bad angels on its sides signifying LLMs receive values from the internet)
...
(illustration LLM reading and a light bulb going off - other wirtings section will focus on topics LLMs can learn from)
...
(illustration LLM creating writings and other LLM reading it)
...
(illustration - neural network ingesting a word and updating weights)
