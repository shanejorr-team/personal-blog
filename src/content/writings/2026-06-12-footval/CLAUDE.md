...
This writing folder contains the draft blog post about Footval, Shane's LLM model evaluation. The post source is `index.mdx`.

## Related Project

Footval lives at `/Users/shaneorr/projects/personal-blog/footval` within this repository. Treat that directory as the source of truth for the benchmark methodology, implementation, and generated outputs used by this post.

Read `/Users/shaneorr/projects/personal-blog/footval/CLAUDE.md` before making substantive changes to the post, especially if you need to describe the benchmark design or interpret outputs.

## What Footval Is

Footval is a subjective, multi-model LLM evaluation. Candidate models answer a fixed prompt about NFL strategy recommendations, Bayesian priors, and a plotting script. The system evaluates model responses through generated artifacts, deterministic checks, and a pairwise LLM-judge tournament (every pair compared head-to-head; there is no separate 1–5 scoring round).
...
`/Users/shaneorr/projects/personal-blog/footval/outputs/blog_output.ipynb`
...
1. `footval/outputs/blog_output.ipynb` builds the charts with Altair and
   ...
   `/Users/shaneorr/miniconda3/bin/jupyter nbconvert --to notebook --execute --inplace footval/outputs/blog_output.ipynb`
2. `index.mdx` imports each `*.svg` with `?raw` and passes it to
   `src/components/footval/StaticChart.astro`, which inlines the SVG inside a
...
- If the outline has placeholders such as `[Insert ...]`, fill them from the notebook outputs or the relevant CSVs under `/Users/shaneorr/projects/personal-blog/footval/outputs/data/`.
