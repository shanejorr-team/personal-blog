...
<InlineFigure src={photo} alt="Description" caption="Optional caption text" />
```
...
<InlineFigure src={illustration} alt="Description" caption="Optional caption text" />
```
...
**Featured System:**
- `homepage_featured`: Controls only the hero image on the homepage
  - `1` = This photo is the homepage hero image
  - `0` = Not the hero image
- `category_featured`: Controls which photos appear on portfolio pages and their display order
  - `0` = Not featured (won't appear on portfolio pages)
  - `1` = Category navigation thumbnail (one per category, also highest priority in grid)
  - `2` = Featured on portfolio - high priority
...
- **Category pages** (`/portfolio/[category]`): Shows ALL photos where `category_featured > 0`
  - Photos sorted by `category_featured` priority (ascending)
...
- Homepage featured (1 for hero image, 0 otherwise)
...
-- Make photo the homepage hero
UPDATE photos SET homepage_featured = 1 WHERE filename = 'photo.jpg';

-- Remove hero status (set another photo as hero first)
UPDATE photos SET homepage_featured = 0 WHERE homepage_featured = 1;
...
```typescript
// Homepage
getHomepageFeatured(): Photo  // Hero photo (homepage_featured = 1)

// Category pages
getCategoryNavigationPhotos(): Photo[]  // Nav thumbnails (category_featured = 1)
getAllCategoryPhotos(category: string): Photo[]  // Featured photos (category_featured > 0)
